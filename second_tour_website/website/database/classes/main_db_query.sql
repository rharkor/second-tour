...
(
    ...
    constraint fk_candidats_id_serie_serie foreign key (id_serie) references serie(id_serie)
);
...
(
    ...
    constraint fk_matieres_serie foreign key (id_serie) references serie(id_serie),
    constraint fk_matieres_salle foreign key (loge) references salle(id_salle)
);
...
(
    ...
    constraint UNQ_CANDIDAT
        unique (id_candidat),
    constraint fk_choix_matieres_matiere1 foreign key (matiere1) references matieres(id_matiere),
    constraint fk_choix_matieres_matiere2 foreign key (matiere2) references matieres(id_matiere),
    constraint fk_choix_matieres_id_candidat foreign key (id_candidat) references candidats(id_candidat)
);
...
(
    ...
    constraint fk_creneau_id_candidat foreign key (id_candidat) references candidats(id_candidat),
    constraint fk_creneau_id_matiere foreign key (id_matiere) references matieres(id_matiere),
    constraint fk_creneau_id_salle foreign key (id_salle) references salle(id_salle)
);

create table if not exists horaires
(
    id_horaires   int auto_increment
        primary key,
...
(
    ...
    constraint fk_liste_matiere_id_matiere foreign key (id_matiere) references matieres(id_matiere)
);
...
(
    ...
    id_professeur int          null,
    constraint UNQ_UTILISATEURS_email
        unique (email, admin),
    constraint fk_utilisateurs_id_professeur foreign key (id_professeur) references professeur(id_professeur)
);

create index id_serie
    on candidats (id_serie);
